...
function nodes(graph, node) {
  const list = graph.reduce((p, c) => {
    c.origin === node &&
      p.push({ destination: c.destination, config: { ...c } });
    return p;
  }, []);
  return list;
}
...
function checkLayoverCriteria(path, startDate) {
  ...
  for (let i = 0; i < path.length - 1; i++) {
    if (
      typeof path[i] === 'object' &&
      typeof path[i + 1] === 'object' &&
      path[i + 1].config.departureTimeStamp - path[i].config.arrivalTimeStamp <=
        1800000
    ) {
      lowerLayover = true;
      break;
...
function isSameDestination(links) {
  ...
  for (let i = 0; i < links.length; i++) {
    const keyName =
      links[i] && links[i].destination ? links[i].destination : links[i];
...
export function findPaths({
  ...
  criteria = { origin: 'Bengaluru (BLR)', destination: 'Delhi (DEL)' }
}) {
  return filterPaths(
    paths({
      graph: flights,
